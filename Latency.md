...
In practice, we accept that the shared states of the applications can be a little out of sync with each other in order to get acceptable responsiveness and a simulation that works at all. The lack of synchronized game state creates other problems we'll discuss later.
