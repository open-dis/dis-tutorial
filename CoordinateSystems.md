...
At this point the "what coordinate system?" question seems to have a simple answer. Simplicity is a virtue in programming, so we'll describe the game world using a coordinate system that assumes the world is infinitely flat in all directions. World of Warcraft, for example, can lay out the cities and roads used in their world on a surface that's infintely flat, and we'll pick a Cartesian master coordinate system that can be used to describe the position of everything in the world. We'll pick a place to put the origin, and then we can describe the position of any feature in the world fairly directly. This makes the math involved not so bad. 

We can also use other coordinate systems that are helpful for an individual entity. We can, for example, set up other local coordinate systems that make physics and mathematical operations simpler. See figure x.

<img src="images/coordinateSystem/multipleCoordinateAxes.jpg"/>

There's a master, global coordinate system, but every entity may also set up another coordinate system that uses itself as the origin, because this usually makes various calculations simpler. There's some math involved to express a new coordinate system in terms of the master coordinate system, but it's not all that bad. If you didn't sleep through the linear algebra class you can probably pull it off.
...
Entities are usually positioned on the earth's ellipsoid-shaped surface. If we're trying to realistically model naval warfare then we won't be able to see a ship 100 km away from our ship; it's below the horizon. Using a single, unified, flat coordinate system would not be realisitic and the simulation wouldn't provide good training if the exercise is conducted on that large of a geographic scale. 

There are other problems. In an LVC environment, we're trying to pull together information from multiple sources. The location of buildings or roads is often provided by maps, and the maps are often using latitude and longitude. These days there are petabytes of open source information on the web that's georeferenced, from coffee shop locations and roads to cultural or tribal information. The data's position is usually expressed in latitude and longitude. The Army often uses Military Grid Reference System (MGRS) to describe the position of units. And, as mentioned above, we've got a game graphics package which we're also using to describe the position of entities, and the graphics system coordinate system is flat, rectilinear, and may have its origin tied to a latitude and longitude.  We may also be trying to describe the orbit of a satellite, or the relative positions of five manuevering aircraft. 

Imagine a virtual vehicle driving through a town. The vehicle is being rendered in a game engine that's using a conventional cartestian coordinate system, which assumes the world is flat and rectilinear. We're using the web as a source of position data for real items in the world, and as anyone who has used Google Maps can attest, those positions are almost always described in terms of latitude, longitude, and altitude. We want to be able to look up while inside the game and see the position of a real satellite overhead, and the position of that satellite's orbit may be described using Keplerian orbital elements. We also want to arrive at a location that's described in MGRS terms. We may also be shot at by a simulated artillery fire system that's 30 KM away, and the curvature of the earth can't be ignored.
...
So what coordinate system should be used to integrate all these sources?

DIS is used in many domains, including sea, subsurface, air, land, and space.  If the simulation's geographic extent is large enough the curvature of the earth can't be ignored. A simulation limted to land operations might choose MGRS. This does not work well for aircraft simulations, where we usually want to do physics calculations in a rectilear system. Naval operations might choose latitude/longitude, but this does not work well for air operations or space operations.  Also, for reasons of convienence we'd like to use a coordinate system that makes local physics calculations easy, or easier. We should also settle on either metric or English units. These goals are in conflict with each other, and some tradeoffs must be made.

DIS chose to use a Cartesian coordinate system with its origin at the center of the earth, and to use meters as the unit of measurement.  The X-axis of this coordinate system points out from the center of the earth and intersects the surface of the earth at the equator and prime meridian. The Y-axis likewise intersects the earth's surface at the equator, but at 90 degrees east longitude. The Z-axis points up through the north pole. This coordinate system rotates wtih the earth; it is sometimes called "Earth-Centered, Earth-Fixed" (ECEF). There's another element to this, a mathematical model for the shape of the surface of the earth. A popular model for the shape of the earth is called WGS-84; that's what Global Positioning System (GPS) uses.
