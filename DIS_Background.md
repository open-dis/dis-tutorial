...
The graphics portion of the shared virutal world is a subject unto itself. There are many ways to draw the 3D images we see on the screen, from OpenGL and Direct3D to higher level scene graphs, such as OpenInventor or X3D. At a high level objects can be rendered with the aid of a 3D game engine such as Lumberyard or Unity. We will be essentially ignoring the graphical display portion of virtual worlds, and focus on what the network and the messages sent over it are doing. It's simply too large of a topic to cover. 
