...
The graphics portion of the shared virutal world is a subject unto itself. There are many ways to draw the 3D images we see on the screen, from OpenGL and Direct3D to higher level scene graphs, such as OpenInventor or X3D. At a high level objects can be rendered with the aid of a 3D game engine such as Lumberyard or Unity. A DIS application may have rudimentary graphics instead of photorealistic 3D graphics. Many DIS applications are map-oriented, and the graphics consist only of icons that represent entities moving on a map.  

We will essentially ignore the graphical display portion of virtual worlds, and focus on what the network and the messages sent over it are doing. It's simply too large of a topic to cover in this context. 

Before the application could be implemented there needed to be a way to exchange *state information* between simulators. State information is data about a participant in the simulation. In the case of a tank, the state information may include its position, the direction it is facing, how fast it is moving, whether it is on fire, and which way the turret is pointing. It's this state information, and the format that it is exchanged in, that DIS standardizes. Once the state information is exchanged the graphics components we just handwaved about above can be used to draw the scene that the users see.
